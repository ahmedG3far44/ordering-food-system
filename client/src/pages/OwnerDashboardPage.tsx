...
const OwnerDashboardPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<"orders" | "sales" | "settings">(
    "orders",
  );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const restaurantIdMap = new Map(
        myRestaurants.map((r) => [r._id || r.id, r.name]),
      );
        ...
        return {
          ...
          restaurant: order.restaurant || {
            ...
            name:
              ...
              restaurantIdMap.get(order.restaurantId) ||
              "Restaurant",
            address: "",
          },
        };
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesRestaurant =
      filterRestaurant === "all" || orderRestaurantId === filterRestaurant;
    const matchesStatus =
  ...
  if (!user) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12 font-mono text-center">
        ...
        <Link to="/login" className="nb-button bg-primary text-white px-6 py-2">
          Login
        </Link>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 font-mono">
        <div className="grid grid-cols-3 gap-6 mb-12">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="bg-slate-100 border-3 border-primary h-32 animate-pulse"
            />
          ))}
        </div>
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="bg-slate-100 border-3 border-primary h-24 animate-pulse"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-black text-primary uppercase font-mono">
            Dashboard
          </h1>
        ...
        <div className="flex gap-2">
          <Link
            ...
            className="nb-button bg-primary text-white px-4 py-2 flex items-center gap-2"
          >
            <Store size={18} />
            Restaurants
          </Link>
        ...
        <div className="flex items-center gap-4 mb-8 overflow-x-auto pb-2">
          {restaurants.map((res) => (
            <button
              ...
              className={`px-4 py-2 border-2 font-bold text-sm uppercase whitespace-nowrap ${
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {restaurants.map((res) => (
                  <option key={res.id} value={res._id || res.id}>
                    {res.name}
                  </option>
                ))}
              </select>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      ...
                      <th className="text-center py-3 text-xs font-bold text-slate-500 uppercase">
                        Status
                      </th>
                      <th className="text-left py-3 text-xs font-bold text-slate-500 uppercase">
                        Date
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 text-center">
                            <span
                              className={`inline-flex items-center gap-1 font-bold text-xs uppercase px-3 py-1 border-2 ${status.color}`}
                            >
                              <StatusIcon
                                size={12}
                                className={status.color.split(" ")[0]}
                              />
                              {status.label}
                            </span>
                          </td>
                          ...
                          <td className="py-3 text-center">
                            <div className="flex justify-center gap-1">
                              {order.status === "PENDING" && (
                                <button
                                  onClick={() =>
                                    handleStatusUpdate(id, "PREPARING")
                                  }
          ...
          <div className="flex flex-wrap items-center gap-4 mb-8">
            ...
            {(["7days", "30days", "90days"] as DateFilter[]).map((filter) => (
              <button
                ...
                className={`px-4 py-2 border-2 font-bold text-sm uppercase ${
                  dateFilter === filter
                    ? "bg-primary text-white border-primary"
                    : "border-slate-200 text-slate-500 hover:border-primary"
                }`}
          ...
          {colors.map((c) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-5 h-5 rounded-full ${c.bg} border-2 border-white`}
                />
                <span className="font-black uppercase">{c.label}</span>
              </div>
              {color === c.value && (
                <span className="text-[10px] font-black uppercase bg-white text-primary px-2 py-1">
                  Active
                </span>
              )}
            </button>
